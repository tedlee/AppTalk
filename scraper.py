#!/usr/bin/env python
''' iOS AppStore Review Scraper

    Special thanks to:
    "Scraping AppStore Reviews" blog by Erica Sadun
...
def getReviews(appStoreId, appId):
	...
	if len(reviews)!=len(stars):
		print "Mismatch between list sizes of reviews and stars."
		raise SystemExit
	output = []
...
def _getReviewsForPage(appStoreId, appId, pageNo):
	...
	try:
		...
		print "Difficulty connecting to the AppStore. Timeout!"
		raise SystemExit
	reviews=[]; stars=[]; topics=[]
	try:
		...
		u.close()
		return (reviews, stars, topics) #Records the review, topic and star rating of the review
...
def _print_reviews(reviews, country):
	...
	testString = ""
	    ...
	    jsonLine+= "{"
	    jsonLine+= "\"stars\": \"" + str(review["rank"]) + "\", " + "\"country\": \"" + str(country) + "\","
	    jsonLine+= "\"title\": \"" + review["topic"] + "\"," + "\"body\": \"" + review["review"] + "\""   
	    
	    jsonLine+= "}"
	    count+= 1

	    try:
	    	print " %s" % (review["review"])
	    except UnicodeEncodeError:
		print "Unicode charater is foobard"
            sumRank += review["rank"]
	jsonLine+= "]}"
	...
	try:
		...
		print "Characters are fubar" 
	return (len(reviews), sumRank)
    else:
        return (0, 0)

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='AppStoreReviewsScrapper command line.', epilog='To get your application Id look into the AppStore link to you app, for example http://itunes.apple.com/us/app/bump/id305479724?mt=8 - app Id is the number between "id" and "?mt=0"')
    ...
    if args.list:
        ...
        if (country=="All"): #Runs through all country stores
